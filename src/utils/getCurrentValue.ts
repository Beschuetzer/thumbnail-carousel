...
export function getCurrentValue<T>(
  valueTuple: CarouselElementValue<T> | undefined,
  defaultValue: T,
  ...
  sorted = [...valueTupleToUse]?.sort((a, b) => {
    const priority = ["max-width", "min-width", undefined] as (
      | CarouselElementValueType
      | undefined
    )[];
