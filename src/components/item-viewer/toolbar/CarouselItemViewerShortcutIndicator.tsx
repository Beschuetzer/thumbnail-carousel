...
import {
  CLASSNAME__DISPLAY_NONE,
  CLASSNAME__ITEM_VIEWER_SHORTCUT_INDICATOR,
} from "../../../constants";
import { useBusinessLogic } from "../../../hooks/useBusinessLogic";
import { KeyInput } from "../../../types";

export type CarouselItemViewerShortcutIndicatorPosition =
  | "left"
  | "center"
  | "right";
...
export const CarouselItemViewerShortcutIndicator = (
  props: CarouselItemViewerShortcutIndicatorProps,
) => {
  const {
    actionName = "",
    children,
  ...
  const hiddenClassName = useMemo(
    () => (isVisible ? "" : CLASSNAME__DISPLAY_NONE),
    [isVisible],
  );
  return (
    <div
      ...
      style={{
        ...stylingLogic.getCarouselShortcutIndicatorContainerStlye(showButton),
        ...style,
      }}
    >
      {hideShortcut ? null : (
        ...
        >
          <span>{capitalize(actionName)}</span>
          {shortcuts.length > 0 ? (
            <span>&nbsp;({getShortcutsString(shortcuts)})</span>
          ) : null}
        </div>
      )}
      {children}
