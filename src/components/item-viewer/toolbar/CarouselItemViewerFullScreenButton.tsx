...
type CarouselItemViewerFullscreenButtonProps =
  {} & CarouselItemViewerButtonProps &
    Pick<CarouselItemViewerToolbarProps, "videoRef">;
export const CarouselItemViewerFullscreenButton = forwardRef<
  any,
  CarouselItemViewerFullscreenButtonProps
>((props, ref) => {
  const {
    actionName = "",
  ...
  const { svgHref, style: buttonStyle } =
    elementStylings?.fullscreenButton || {};
  const fillColor = optionsLogic.getButtonColor(
    CarouselElement.fullscreenButton,
  );
      ...
      {!!svgHref ? (
        <CarouselItemViewerCustomButton
          ...
          style={stylingLogic.getCarouselElementChildSizeStlye({
            buttonName: CarouselElement.fullscreenButton,
            subElementName: null,
          })}
