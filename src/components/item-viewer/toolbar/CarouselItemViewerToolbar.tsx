...
import {
  ...
  ToolbarPreviewDirection,
} from "./CarouselItemViewerToolbarPreview";
...
export const CarouselItemViewerToolbar = forwardRef<
  HTMLElement,
...
>((props, ref) => {
  ...
  const [previewDirection, setPreviewDirection] = useState(
    ToolbarPreviewDirection.none,
  );
  const [isPreviousItemPreviewLoaded, setIsPreviousItemPreviewLoaded] =
    useState(false);
  ...
  const [showFullscreenButtonPopup, setShowFullscreenButtonPopup] =
    useState(false);
  ...
  const [showSeekForwardButtonPopup, setShowSeekForwardButtonPopup] =
    useState(false);
  const [showSeekBackwardButtonPopup, setShowSeekBackwardButtonPopup] =
    useState(false);
    ...
    keyboardShortcuts: [
      {
        ...
        action: toolbarActionsLogic.isPauseSeparate
          ? handleKeyboardPlay
          : handlePlayPauseUnited,
      },
      {
  ...
  useUpdateTimeString({
    currentItemInInstance: currentItem,
    setTimeStrings,
    videoRef,
  });
  ...
  const hideToolbar = useCallback(() => {
    if (
      ...
      itemContainerRef.current.classList?.add(
        CLASSNAME__ITEM_CONTAINER_NO_TOOLBAR,
      );
    }
  ...
  const showToolbar = useCallback(() => {
    if (itemContainerRef?.current) {
      itemContainerRef.current.classList?.remove(
        CLASSNAME__ITEM_CONTAINER_NO_TOOLBAR,
      );
    }
  }, [itemContainerRef]);
  ...
  const handleAutoHide = useCallback(
    (e?: MouseEvent) => {
      ...
      if (
        (!isFullscreenMode && isVideo && !isVideoPlaying) ||
        optionsLogic.autoHideToolbarDuration === AUTO_HIDE_DISABLED_VALUE
      )
        return;
  ...
  const handleMouseLeave = useCallback(
    (e: MouseEvent) => {
      ...
      const point = getPoint(e);
      const isInVideoBox = getIsPointInsideElement(
        point,
        (videoRef?.current || imageRef?.current) as HTMLElement,
      );
      const isVideoPlaying = !!imageRef?.current
        ? true
        : getIsVideoPlaying(videoRef?.current);
  ...
  useEffect(() => {
    ...
    const boundDisplayCloseButton = handleDisplayPopup.bind(
      null,
      true,
      setShowCloseButtonPopup,
    );
    const boundHideCloseButton = handleDisplayPopup.bind(
      null,
      false,
      setShowCloseButtonPopup,
    );
    const boundDisplayFullscreenButton = handleDisplayPopup.bind(
      null,
      true,
      setShowFullscreenButtonPopup,
    );
    const boundHideFullscreenButton = handleDisplayPopup.bind(
      null,
      false,
      setShowFullscreenButtonPopup,
    );
    const boundDisplayPauseButton = handleDisplayPopup.bind(
      null,
      true,
      setShowPauseButtonPopup,
    );
    const boundHidePauseButton = handleDisplayPopup.bind(
      null,
      false,
      setShowPauseButtonPopup,
    );
    const boundDisplayPlayButton = handleDisplayPopup.bind(
      null,
      true,
      setShowPlayButtonPopup,
    );
    const boundHidePlayButton = handleDisplayPopup.bind(
      null,
      false,
      setShowPlayButtonPopup,
    );
    const boundDisplaySeekForwardButton = handleDisplayPopup.bind(
      null,
      true,
      setShowSeekForwardButtonPopup,
    );
    const boundHideSeekForwardButton = handleDisplayPopup.bind(
      null,
      false,
      setShowSeekForwardButtonPopup,
    );
    const boundDisplaySeekBackwardButton = handleDisplayPopup.bind(
      null,
      true,
      setShowSeekBackwardButtonPopup,
    );
    const boundHideSeekBackwardButton = handleDisplayPopup.bind(
      null,
      false,
      setShowSeekBackwardButtonPopup,
    );
    ...
    function handleDisplayPopup(
      ...
      showPopupSetter: React.Dispatch<React.SetStateAction<boolean>>,
      e: MouseEvent,
    ) {
      stopPropagation(e);
    ...
    if (closeButtonRef?.current) {
      closeButtonRef.current.addEventListener(
        "mouseenter",
        boundDisplayCloseButton,
      );
      closeButtonRef.current.addEventListener(
        "mouseleave",
        boundHideCloseButton,
      );
    }

    if (fullscreenButtonRef?.current) {
      fullscreenButtonRef.current.addEventListener(
        "mouseenter",
        boundDisplayFullscreenButton,
      );
      fullscreenButtonRef.current.addEventListener(
        "mouseleave",
        boundHideFullscreenButton,
      );
    }

    if (nextButtonRef?.current) {
      nextButtonRef.current.addEventListener(
        "mouseenter",
    ...
    if (pauseButtonRef?.current) {
      pauseButtonRef.current.addEventListener(
        "mouseenter",
        boundDisplayPauseButton,
      );
      pauseButtonRef.current.addEventListener(
        "mouseleave",
        boundHidePauseButton,
      );
    }

    if (playButtonRef?.current) {
      playButtonRef.current.addEventListener(
        "mouseenter",
        boundDisplayPlayButton,
      );
    ...
    if (previousButtonRef?.current) {
      previousButtonRef.current.addEventListener(
        "mouseenter",
        handleMouseEnterPreviousButton,
      );
      previousButtonRef.current.addEventListener(
        "mouseleave",
        handleMouseLeaveButton,
    ...
    if (seekBackwardButtonRef?.current) {
      seekBackwardButtonRef.current.addEventListener(
        "mouseenter",
        boundDisplaySeekBackwardButton,
      );
      seekBackwardButtonRef.current.addEventListener(
        "mouseleave",
        boundHideSeekBackwardButton,
      );
    }

    if (seekForwardButtonRef?.current) {
      seekForwardButtonRef.current.addEventListener(
        "mouseenter",
        boundDisplaySeekForwardButton,
      );
      seekForwardButtonRef.current.addEventListener(
        "mouseleave",
        boundHideSeekForwardButton,
      );
    }
    return () => {
      if (closeButtonRefCopy) {
        closeButtonRefCopy.removeEventListener(
          "mouseenter",
          boundDisplayCloseButton,
        );
        closeButtonRefCopy.removeEventListener(
      ...
      if (fullScreenButtonRefCopy) {
        fullScreenButtonRefCopy.removeEventListener(
          "mouseenter",
          boundDisplayFullscreenButton,
        );
        fullScreenButtonRefCopy.removeEventListener(
      ...
      if (nextButtonRefCopy) {
        nextButtonRefCopy.removeEventListener(
          "mouseenter",
          handleMouseEnterNextButton,
        );
        nextButtonRefCopy.removeEventListener(
      ...
      if (pauseButtonRefCopy) {
        pauseButtonRefCopy.removeEventListener(
          "mouseenter",
          boundDisplayPauseButton,
        );
        pauseButtonRefCopy.removeEventListener(
      ...
      if (playButtonRefCopy) {
        playButtonRefCopy.removeEventListener(
          "mouseenter",
          boundDisplayPlayButton,
        );
        playButtonRefCopy.removeEventListener(
      ...
      if (previousButtonRefCopy) {
        previousButtonRefCopy.removeEventListener(
          "mouseenter",
          handleMouseEnterPreviousButton,
        );
        previousButtonRefCopy.removeEventListener(
      ...
      if (seekBackwardButtonRefCopy) {
        seekBackwardButtonRefCopy.removeEventListener(
          "mouseenter",
          boundDisplaySeekBackwardButton,
        );
        seekBackwardButtonRefCopy.removeEventListener(
      ...
      if (seekForwardButtonRefCopy) {
        seekForwardButtonRefCopy.removeEventListener(
          "mouseenter",
          boundDisplaySeekForwardButton,
        );
        seekForwardButtonRefCopy.removeEventListener(
  ...
  return (
    ...
    >
      {isVideo &&
      !isFullscreenMode &&
      optionsLogic.useDefaultVideoControls ? null : (
        <div style={stylingLogic.toolbarOuterContainerStyle}>
          {videoRef ? (
            ...
            {videoRef ? (
              <div className={CLASSNAME__TOOLBAR_LEFT}>
                <CarouselItemViewerPlayButton
                  actionName="Play"
                  isShortcutVisible={
                    showPlayButtonPopup && !isProgressBarMouseDownRef?.current
                  }
                ...
                <CarouselItemViewerPauseButton
                  actionName="Pause"
                  isShortcutVisible={
                    showPauseButtonPopup && !isProgressBarMouseDownRef?.current
                  }
                  isVisible={isVideoPlaying}
          ...
          <CarouselItemViewerToolbarPreview
            itemToShow={
              items[getPreviewItemIndex(ToolbarPreviewDirection.previous)]
            }
          ...
          <CarouselItemViewerToolbarPreview
            itemToShow={
              items[getPreviewItemIndex(ToolbarPreviewDirection.next)]
            }
