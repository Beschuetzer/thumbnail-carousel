...
export const CarouselItems = forwardRef<HTMLDivElement, CarouselItemsProps>(
  (props, ref) => {
    const {
      interItemSpacing,
      isLastPage,
    ...
    return (
      ...
      >
        <div
          ref={ref}
          style={stylingLogic.getCarouselItemsInnerContainerStyle(
            interItemSpacing,
            translationAmount,
            isLastPage,
            translationAmountChangeRef,
        ...
        >
          {items.map((item, index) => (
            <CarouselItem key={index} index={index} {...item} />
          ))}
        </div>
      </div>
    );
