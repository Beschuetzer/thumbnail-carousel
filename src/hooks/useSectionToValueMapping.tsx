...
const NEXT_SECTION_OFFSET = 0.0000000000000001;
...
export type UseSectionToValueMappingInput = {} & Pick<
  CarouselItemViewerToolbarProps,
  "videoRef"
>;
const EXAMPLE_SENTENCE =
  "Please check CarouselVideoOptions.sections in CarouselVideo.tsx for examples.";
 ...
 **/
export const useSectionToValueMapping = (
  input: UseSectionToValueMappingInput,
) => {
  const { videoRef } = input;
  ...
  const mapSectionRanges = useCallback(() => {
    ...
    let amountBefore = 0;
    const videoDuration =
      (videoRef?.current?.duration || 0) * NUMBER_OF_MS_IN_A_SECOND;
    ...
    try {
      for (let index = 0; index < sections.length; index++) {
        ...
        const start =
          indexToUse === 0
            ? 0
            : sectionToValueMappingRef.current[indexToUse - 1]?.end +
              NEXT_SECTION_OFFSET;
        const end =
          indexToUse === sections.length - 1 ? 1 : amountBefore / videoDuration;

        if (end > 1)
          alert(
            `Section ${
              index + 1
            } ends after the video's end.  Please check the sections object for this video.`,
          );
      ...
      alert(
        `Developer Warning: There is an issue with this item's sections.  ${EXAMPLE_SENTENCE}`,
      );
    }
  ...
  const validateSections = useCallback(() => {
    ...
    const isString =
      typeof sections?.[0]?.[1] === "string" ||
      typeof sections?.[0]?.[1] === "undefined";
      ...
      if (
        sections.some(
          (section) =>
            ...
            typeof section?.[1] !== "undefined",
        )
      ) {
        alert(
          `Developer Warning: Expecting a duration for sections.  Either different types of values are being used or an expected duration value was omitted from a section.  ${EXAMPLE_SENTENCE}`,
        );
        throw new Error();
      }
    }
